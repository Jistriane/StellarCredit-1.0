...
// Carregar também variáveis de .env.contract (raiz do projeto)
try {
  const contractEnvPath = path.resolve(__dirname, '..', '.env.contract');
  if (fs.existsSync(contractEnvPath)) {
    const content = fs.readFileSync(contractEnvPath, 'utf-8');
    content.split(/\r?\n/).forEach((line) => {
    ...
    // Mapear STELLAR_CREDIT_CONTRACT_ID -> CONTRACT_ADDRESS se necessário
    if (!process.env.CONTRACT_ADDRESS && process.env.STELLAR_CREDIT_CONTRACT_ID) {
      process.env.CONTRACT_ADDRESS = process.env.STELLAR_CREDIT_CONTRACT_ID;
    }
  }
} catch (e) {
  // Não falhar inicialização por erro de leitura
}
